...
* \file
*
* This header defines the \ref TextPresentation class.
***********************************************************************************************************************/

/* .. sphinx-project ineapp */

#ifndef TEXT_PRESENTATION_H
#define TEXT_PRESENTATION_H
...
/**
 * This class provides a visual for the Ld::TextElement class with full support for presenting a Ld::TextElement within
 * the document view of a program.
 */
class APP_PUBLIC_API TextPresentation:public         LeafPresentation,
                                      public virtual Ld::TextVisual,
                                      private        TextPresentationHelper {
    Q_OBJECT

    public:
        TextPresentation();

        ~TextPresentation() override;

        /**
         * Static method that creates an instance of a \ref TextPresentation class.
         *
    ...
    signals:
         ...
         *
         *     * \ref TextElement::setText(const QString&)
         *
         * \param[in] newText The new text held by the element.
         */
        void textHasChanged(QString newText);

    protected slots:
        /**
         * Slot that is triggered when the text tracked by this element is changed.  You can overload this slot if you
         * wish to receive notification of a text change event.  This slot is triggered by the following methods:
         *
         *     * Ld::TextElement::setText(const QString&)
         *
         * The slot primarily exists to allow call-backs from the Ld::TextVisual to occur across thread
         * boundaries.
         *
         * \param[in] newText The new text to dsiplay.
         */
        virtual void processTextChange(const QString& newText);
    ...
    protected:
         ...
         *         if the text region position represents the top-left corner.  This version returns false.
         */
        bool textPositionIsBaseline() const override;
    ...
    private:
         ...
         */
        void textChanged(const QString& newText) final;

        /**
         * Method that can be called to clear the list of simple graphics items.
         *
         * \param[in] startingAreaId The first presentation area to be cleared.
         */
        void clearGraphicsItems(unsigned long startingAreaId = 0);

        /**
         * Method that can be called to locate the graphics item associated with a specific character offset.
         *
         * \param[in] characterOffset    The zero based offset into the graphics item.
         *
         * \param[in] itemStartingOffset An optional pointer to a location holding the starting character offset
         *                               representing this graphics item.
         *
         * \return Returns the zero based index into the graphicsItems array.  A value of static_cast<unsigned long>(-1)
         *         is returned if the position is invalid.
         */
        unsigned long graphicsItemAtOffset(
            unsigned long  characterOffset,
            unsigned long* itemStartingOffset = Q_NULLPTR
        ) const;

        /**
         * List of graphics items used to present the text.
         */
        QList<EQt::GraphicsTextItem*> graphicsItems;

        /**
         * List of starting line offsets associated with each graphics item.
         */
        QList<unsigned long> startingLineOffsets;
};
